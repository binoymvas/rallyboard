...
#from openstack_dashboard.dashboards.sidecar.events.model import *
...
ADD_USER_URL = "horizon:rally_dashboard:events:update"
...
class CreateProjectInfoAction(workflows.Action):
    ...
    domain_name = forms.CharField(label=_("Domain Namei."), required=False, widget=forms.HiddenInput())
    image_ref = forms.CharField(label=_("Image Ref"), max_length=64) #name = forms.CharField(label=_("Name"), max_length=64)
    flavor_ref = forms.CharField(label=_("Flavor Ref"), required=False) #description = forms.CharField(label=_("Flavor Ref"), required=False) 
    #enabled = forms.BooleanField(label=_("Enabled"), required=False, initial=True)
    ...
    def __init__(self, request, *args, **kwargs):
        ...
        if keystone.VERSIONS.active >= 3:
            ...
            self.fields["domain_id"].widget = forms.HiddenInput() #readonlyInput
            self.fields["domain_name"].widget = forms.HiddenInput() #readonlyInput

    class Meta(object):
        name = _("Config Information")
...
class UpdateProjectMembersAction(workflows.MembershipAction):
    def __init__(self, request, *args, **kwargs):
        ...
        try:
            test_list = sidecar.events.tests_list(project_id=args[0]['enabled'])
        except Exception:
	    ...
	    #self.fields[field_name].widget = HiddenInput()	
            ...
            self.fields[field_name].choices = tests_list
            self.fields[field_name].initial = []

        # Figure out users & roles
	project_id = '37579cedc7fd4060801bb08035ef5ce4'
        if project_id:
            try:
...
class UpdateProjectMembers(workflows.UpdateMembersStep):
    ...
    members_list_title = _("Selected tests")
    no_available_text = _("No tests found.")
    no_members_text = _("No tests.")
...
class UpdateProject(CommonQuotaWorkflow):
    ...
    #default_steps = (UpdateProjectInfo, UpdateProjectMembers, UpdateProjectQuota)
    ...
    def __init__(self, request=None, context_seed=None, entry_point=None,
        ...
        if PROJECT_GROUP_ENABLED:
            ...
            #self.default_steps = (UpdateProjectInfo, UpdateProjectMembers, UpdateProjectGroups, UpdateProjectQuota)
    ...
    def _update_project(self, request, data):
        ...
        for field_key in data.keys():
            if data[field_key] == None:
                continue
            if len(data[field_key]) > 0:
                for test_id in data[field_key]:
                    try:
                        sidecar.events.update_test(id=test_id, test_added='1')
                    except exceptions.Conflict:
                        msg = _('Test with id "%s" is already used.') % test_id
                        self.failure_message = msg
                        return
                    except Exception:
                        exceptions.handle(request, ignore=True)
    ...
    def _update_project_members(self, request, data, project_id):
        # update project members
        print(" in the _update_project_members---------------------------")
        users_to_modify = 0
